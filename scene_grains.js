...
const createScene = async function () {
    ...
    poseNet.on('pose', function (results) {
        if (results.length > 0) {
            ...
            sphereLeft.position.y = map(poses.leftWrist.y, video.height, 0, 0.8, 1.8);
            sphereRight.position.x = map(poses.rightWrist.x, 100, video.width, 0.05, 0.8);
            ...
            //console.log(poses.leftWrist.y+ " " + window.innerHeight + " " + posY);
            //console.log(poses.rightWrist.x+ " " + window.innerWidth + " " + posX);
              //grains(posX,posY); // also depends on video stream size, but is scaled across screen size
             grains(poses.rightWrist.x,poses.leftWrist.y); // depends on video size
