...
const createScene = async function () {
    ...
    // ---------- Audio Context ------------------//
        ...
        var setFrequency = () => {
            ...
            oscillator.frequency.setTargetAtTime(pitchDistance, ctx.currentTime, 0.01);
        };
        ...
        var setNoteFrequency = () => {
            var pitchDistance = BABYLON.Vector3.DistanceSquared(sphereRight.position, pitchAntennaPosition);
            var volumeDistance = BABYLON.Vector3.DistanceSquared(sphereLeft.position, volumeAntennaPosition);

            //const synth = new Tone.Synth().toDestination();

            console.log(Tone.Frequency(pitchDistance).toNote());
            
            
            //console.log(" right wrist x "+ posX + " left wrist y "+posY);
            //tweakPercentsPitchDistance(pitchDistance, volumeDistance, setanchor = true); // TODO check false value condition
            notename = centsToNoteName(anchorcents);
            //playSoundPitchDistance(pitchDistance,volumeDistance);
    ...
    poseNet.on('pose', function (results) {
        if (results.length > 0) {
            ...
            var pitchDistance = BABYLON.Vector3.DistanceSquared(sphereRight.position, pitchAntennaPosition);
            var volumeDistance = BABYLON.Vector3.DistanceSquared(sphereLeft.position, volumeAntennaPosition);
            console.log("note :"+ Tone.Frequency(pitchDistance).toNote());
            
            Tone.Transport.scheduleOnce((time) => {
                synth.triggerAttackRelease(
                Tone.Frequency(pitchDistance).toNote(), "8n"
                );
              }, "4n" + Tone.now());
            ...
            //setNoteFrequency();
            //setNoteGain();
            //releaseSound(ctx.currentTime+200);
        }
        // else {
        //     releaseSound();
    ...
    //------------------- Grain Param GUI --------------------//
        ...
        var btnViolin = BABYLON.GUI.Button.CreateSimpleButton("but1", "Violin");
        btnViolin.width = "80px"
        btnViolin.height = "20px";
        btnViolin.color = "white";
        btnViolin.cornerRadius = 20;
        btnViolin.background = "green";
        btnViolin.onPointerUpObservable.add(function() {
            //await Tone.start();
            //console.log('tone audio is ready')
            // instr = "violin";
            // myin = instrument_switcher[instr];
            // myloopstart = loopstart_switcher[instr];
            // myloopend = loopend_switcher[instr];
      
        });
        panel.addControl(btnViolin);
